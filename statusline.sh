...
if [[ "$need_recompute" == "true" ]]; then
  ...
  if [[ -n "$day_lo" ]]; then
    ...
    if (( ${#jsonl_files[@]} > 0 )); then
      ...
      # Fable 5 ($10/$50) is the tier above Opus and must be matched before the
      # generic `opus` fallback can't catch it anyway (no `opus` substring), but
      # it's listed first so a new fable variant lands in the right bucket. A
      # missing fable row silently drops every fable record from the daily total
      # via the `model_rate(...) != null` filter — which is exactly the bug that
      # under-reported a heavy Fable 5 day as ~1/4 of the session cost.
